...
def read_modifiedd(file_path, address=None):
    data = []
    count = 0
    lasttime = 0
    pattern = r'\[current_(\d+)\]:.*ModifiedD\s*=\s*([\d.]+),\s*time\s*=\s*(\d+)'
    with open(file_path, 'r') as f:
        for line in f:
            match = re.search(pattern, line)
            if match:
                ...
                if address is not None and addr != address:
                    continue
                modifiedd = float(match.group(2))
                time = int(match.group(3))
                if time < lasttime and time < 1099511627776:
                    count += 1
                lasttime = time
                data.append((time + count * 1099511627776, modifiedd))
    return np.array(data)

def plot_modifiedd(data):
    if len(data) == 0:
        print("No valid ModifiedD data found!")
        return
    time = data[:, 0]
    modifiedd = data[:, 1]
    plt.figure(figsize=(12, 6))
    plt.plot(time, modifiedd, 'b-o', label='ModifiedD', alpha=0.8)
    plt.xlabel('Time')
    plt.ylabel('ModifiedD')
    plt.title('ModifiedD Over Time')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig('data/modifiedd_only.png')
    plt.show()

if __name__ == "__main__":
    file_path = 'data/dataLog.txt'
    ADDRESS = 34698  # 设备地址
    data = read_modifiedd(file_path, address=ADDRESS)
    plot_modifiedd(data)